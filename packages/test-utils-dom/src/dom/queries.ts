...
export class Queries {
  ...
  waitForQuerySelector(selectors: string, params?: TWaitforParams) {
    return buildWaitFor((resolve, reject) => {
      ...
      dom
        ? resolve(dom)
        : reject(`waitForQuerySelector: ${selectors} not found!!`)
    }, params)
  }
  ...
  waitForQuerySelectorAll(selectors: string, params?: TWaitforParams) {
    return buildWaitFor((resolve, reject) => {
      ...
      doms.length
        ? resolve(doms)
        : reject(`waitForQuerySelectorAll: ${selectors} not found!!`)
    }, params)
  }
  ...
  queryAllByText(text: string, selector = '*') {
    return [...Array.from(this._container.querySelectorAll(selector))].filter(
      (node) => {
  ...
  waitForQueryAllByText(text: string, selector = '*', params?: TWaitforParams) {
    return buildWaitFor((resolve, reject) => {
      ...
      doms.length
        ? resolve(doms)
        : reject(`waitForQueryAllByText: ${text} not found!!`)
    }, params)
  }
  ...
  waitForQueryByAttribute(attr: string, value: any, params?: TWaitforParams) {
    return buildWaitFor((resolve, reject) => {
      ...
      dom
        ? resolve(dom)
        : reject(`waitForQueryByAttribute: ${attr} not found!!`)
    }, params)
  }
  ...
  waitForQueryByPlaceholder(text: string, params?: TWaitforParams) {
    return buildWaitFor((resolve, reject) => {
      ...
      dom
        ? resolve(dom)
        : reject(`waitForQueryByPlaceholder: ${text} not found!!`)
    }, params)
  }
