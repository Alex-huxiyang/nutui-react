...
import {
  app as AppLifecycle,
  page as PageLifecycle,
  proxyAppLifeCycle,
} from '@tarojs/test-utils-shared'
...
class ReactTestUtil {
  ...
  async mount<T extends Record<any, any>>(
    ui: FunctionComponent<T> | ComponentClass<T>,
    params: TRenderParams<T> = {}
  ) {
  ...
  html() {
    if (this._inst) {
      if (!document.contains(this._inst.container)) {
        throw new Error(
          'container 节点没有挂载到document上！这将导致WebComponent无法触发生命周期导致无法正常渲染'
        )
      }
